...
class HoaDonTienDaoTaoController extends Controller
{
    //
    public function index(){
        $nghiepdoan = NghiepDoan::orderBy('id','asc')->where('flag','0')->get();
        $nganhang = NganHang::orderBy('id','asc')->where('flag','0')->get();
        return view('admin.template.hoadontiendaotao.index',['nghiepdoan'=>$nghiepdoan,'nganhang'=>$nganhang]);
    }
    public function getdata($id_ndoan){
        ...
        ->where([['id_ndoan',$id_ndoan],['flag_daotao',0],['flag',0]])
        ...
        if(count($hocvien)==0){
            ...
            ->get();
            $tong = $tong[0];
            return view('admin.template.hoadontiendaotao.data',[
                'hocvien'=>$hocvien,
    ...
    public function print(Request $request,$id_ndoan){
        ...
        $trangthai = DB::table('hocvien')->where([['id_ndoan',$id_ndoan],['flag_daotao',0],['flag',0]])->update(['flag_daotao' => 1]);
        $ma_seikyu_tiendaotao = DB::table('hocvien')->where([['id_ndoan',$id_ndoan],['flag_daotao',1],['flag',0]])->update(['ma_seikyu_tiendaotao' => $ma_seikyu]);
        //insert du lieu vao bang seikyu dao tao
        ...
        $xacnhanseikyu->loai_seikyu = 'Tiền đào tạo';
        ...
        $xacnhanseikyu->save();
        return view('admin.template.hoadontiendaotao.inhoadondaotao',[
            'hocvien'=>$hocvien,
            'nghiepdoan'=>$nghiepdoan,
            'nganhang'=>$nganhang,
            'congty'=>$congty,
