...
<div class="row border-bottom bd-lightGray mt-4">
    <div class="col-md-4 text-center text-left-md">
        <h3 class="content_title m-0">Danh sách ngành nghề</h3>
    </div>
    <div class="col-md-8 d-flex flex-justify-center flex-justify-end-md">
        <ul class="breadcrumbs bg-transparent m-0">
            ...
            <li class="page-item"><a class="page-link" href="#">Danh sách ngành nghề</a></li>
        </ul>
    </div>
</div>
            ...
            <div class="col-12">
                <div class="bg-white p-4">
                    <div class="custom_table-wrapper">
                        <a style="float:right;" href="/addnganhnghe" class="image-button primary"><span class="mif-plus icon"></span><span class="caption">Thêm</span></a>
                        ...
                        <table class="table table-border cell-border"    data-table-search-title="Tìm kiếm" data-table-rows-count-title="Hiển thị" id="table1" data-role="table" data-search-wrapper=".custom_table-search" data-rows-wrapper=".custom_table-rows" data-info-wrapper=".custom_table-info" data-pagination-wrapper=".custom_table-pagination" data-pagination-short-mode="true" data-horizontal-scroll="true" data-check="fakse" data-check-style="2" data-rownum="true">
                            ...
                            <tbody>
                                @foreach ($nganhnghe as $item)
                                    <tr>
                                        <td>{{$item->loainganh_jp}}</td>
                                        <td>{{$item->loainganh_vn}}</td>
                                        <td>{{$item->name_jp}}</td>
                                        <td>{{$item->name_vn}}</td>
                                        <td>{{$item->creator}}</td>
                                        <td >
                                            <div  style="text-align: center;color:white;width:77px">
                                                <a href="/editnganhnghe/{{$item->id}}" class="mt-1 button cycle square primary"><span class="mif-wrench"></span></a>
                                                <a onclick="return confirm('Bạn có chắc chắn muốn xóa? Lưu ý sau khi xóa sẽ không phục hồi lại được')" href="/deletenganhnghe/{{$item->id}}" class="mt-1 button cycle square alert btn-delete"><span class="mif-cross"></span></a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
